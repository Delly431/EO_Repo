...
class _Db extends \mysqli
{
	...
	public static function i( $identifier=NULL, $connectionSettings=array() )
	{
		...
		if( !isset( self::$multitons[ $identifier ] ) )
		{
			...
			if ( \IPS\IN_DEV )
			{
				self::$multitons[ $identifier ]->query( "SET sql_mode='STRICT_ALL_TABLES,ONLY_FULL_GROUP_BY'" );
			}
	...
	public function preparedQuery( $query, array $_binds )
	{
		...
		foreach( debug_backtrace( FALSE ) as $b )
		{
			...
			if( isset( $b['class'] ) and !in_array( $b['class'], array( 'IPS\_Db', 'IPS\Db\_Select', 'IPS\Patterns\_ActiveRecord', 'IPS\Patterns\_ActiveRecordIterator' ) ) )
			{
	...
	public function union( $selects, $order, $limit, $group=NULL, $unionAll=FALSE, $flags=0, $where=NULL, $querySelect='*' )
	{
		...
		$query = "SELECT " . ( $flags & static::SELECT_SQL_CALC_FOUND_ROWS ? "SQL_CALC_FOUND_ROWS " : "" ) . $querySelect . " FROM( " . implode( ' ' . $union . ' ', $query ) . ") derivedTable ";
	 ...
	 * @param	string								$table				Table Name
	 * @param	string|array|\IPS\Db\Statement|null	$where				WHERE clause (see \IPS\Db::compileWhereClause for details)
	 * @param	string|null							$order				ORDER BY clause
	 * @param	int|array|null						$limit				LIMIT clause (see \IPS\Db::select for details)
		...
		if( isset( $data['indexes'] ) )
		{
			foreach ( $data['indexes'] as $index )
			{
				if( $index['type'] === 'fulltext' )
				{
					...
					if( ( \IPS\Db::i()->server_version < 50600 and ! mb_strpos( $mysqlVersion, "MariaDB" ) ) OR ( isset( $data['engine'] ) AND !in_array( \strtoupper( $data['engine'] ), array( 'INNODB', 'MYISAM' ) ) ) )
					{
						$data['engine'] = 'MYISAM';
					}
				}
