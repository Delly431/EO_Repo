...
abstract class _Content extends \IPS\Patterns\ActiveRecord
{
	...
	{	
		/* Specifically remove quotes, and any scripts (which someone with HTML posting allowed may have legitimately enabled */
	...
	public static function searchResult( array $indexData, array $authorData, array $itemData, array $containerData = NULL, array $reputationData, $reviewRating, $iPostedIn, $view, $asItem )
	{
		...
		/* Return */		
		return \IPS\Theme::i()->getTemplate( 'system', 'core', 'front' )->searchResult( $indexData, $articles, $authorData, $itemData, $unread, $asItem ? $itemUrl : $objectUrl, $itemUrl, $containerUrl, $containerTitle, $repCount, $showRepUrl, $snippet, $iPostedIn, $view );
	}
	...
	public function likeBlurb()
	{
		...
		if( \IPS\Member::loggedIn()->group['gbw_view_reps'] )
		{
			/* 3 random names */
			$idColumn = static::$databaseColumnId;
			foreach ( \IPS\Db::i()->select( '*', 'core_reputation_index', array( 'app=? AND type=? AND type_id=? AND member_id!=? AND rep_rating=?', static::$application, static::$reputationType, $this->$idColumn, ( \IPS\Member::loggedIn()->member_id ) ?: 0, 1 ), 'RAND()', 3 ) as $rep )
			{
