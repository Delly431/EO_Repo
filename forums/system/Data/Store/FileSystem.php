...
CONTENTS;
		...
		/* Let's attempt to do an atomic update to prevent concurrent writes from causing problems */
		$temp = tempnam( \IPS\TEMP_DIRECTORY, mt_rand() );

		if( !( $fileHandle = @fopen( $temp, 'wb' ) ) )
		{
			/* We couldn't get a file handle on the temp file, so let's just use the file_put_contents method which is not atomic */
		...
		else
		{
			...
			if( !@rename( $temp, $this->_path . '/' . $key . '.' . \IPS\SUITE_UNIQUE_KEY . '.php' ) )
			{
				@unlink( $this->_path . '/' . $key . '.' . \IPS\SUITE_UNIQUE_KEY . '.php' );
				@rename( $temp, $this->_path . '/' . $key . '.' . \IPS\SUITE_UNIQUE_KEY . '.php' );
