	...
	\IPS\Content\Embeddable
{
	...
	public function processForm( $values )
	{
		...
		if ( isset( $values['topic_create_state'] ) )
		{
			...
			if ( static::modPermission( 'hide', NULL, $this->container() ) )
			{
				...
				{
					/* If we're coming from an unapproved state (not hidden) we need to make sure the container gets updated */
					if( $this->approved === 0 )
					{
						$this->onUnhide( TRUE, NULL );
					}
					$this->approved = 1;
				}
			}
	...
	public function canSetBestAnswer( \IPS\Member $member = NULL )
	{
		...
		(
			...
			and
			(
				$member->modPermission( \IPS\forums\Forum::$modPerm ) === -1
				or
				(
