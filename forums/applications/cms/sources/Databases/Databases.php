...
class _Databases extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	...
	public static function databases()
	{
		if ( ! static::$gotAll )
		{
			...
			{
				\IPS\Data\Store::i()->cms_databases = iterator_to_array( \IPS\Db::i()->select( '*', static::$databaseTable )->setKeyField('database_id') );
			}
			
			/* Avoid using SHOW TABLES LIKE / checkForTable() */
            try
            {
                foreach( \IPS\Data\Store::i()->cms_databases as $db )
                {
	...
	public function preLoadWords()
	{
		...
		if ( ! $this->langLoaded )
		{
			...
			\IPS\Member::loggedIn()->language()->words['__indefart_content_db_lang_su_' . $this->id ] = \IPS\Member::loggedIn()->language()->addToStack( 'content_db_lang_ia_' . $this->id );

			\IPS\Member::loggedIn()->language()->words['content_record_comments_title_' . $this->id ] = \IPS\Member::loggedIn()->language()->addToStack( 'content_record_comment_title', FALSE, array( 'sprintf' => array( $this->recordWord( 1, TRUE ) ) ) );
			\IPS\Member::loggedIn()->language()->words['content_record_reviews_title_' . $this->id ] = \IPS\Member::loggedIn()->language()->addToStack( 'content_record_review_title', FALSE, array( 'sprintf' => array( $this->recordWord( 1, TRUE ) ) ) );
			...
			\IPS\Member::loggedIn()->language()->words['content_db_lang_su_' . $this->id . '_pl' ] =  \IPS\Member::loggedIn()->language()->addToStack( 'content_db_lang_pu_' . $this->id );
			\IPS\Member::loggedIn()->language()->words['content_db_lang_su_' . $this->id . '_pl_lc' ] =  \IPS\Member::loggedIn()->language()->addToStack( 'content_db_lang_pl_' . $this->id );
			\IPS\Member::loggedIn()->language()->words['content_db_lang_sl_' . $this->id . '_pl_lc' ] =  \IPS\Member::loggedIn()->language()->addToStack( 'content_db_lang_pl_' . $this->id );
			...
			{
				\IPS\Member::loggedIn()->language()->words['sort_field_' . $id] = \IPS\Member::loggedIn()->language()->addToStack( 'content_field_' . $id );
			}
	...
	public function delete()
	{
		...
		\IPS\Db::i()->delete( 'core_search_index', array( 'index_class=?', "IPS\cms\Records\Comment" . $this->id ) );
		\IPS\Db::i()->delete( 'core_search_index', array( 'index_class=?', "IPS\cms\Records\Review" . $this->id ) );
