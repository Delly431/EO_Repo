...
class _category extends \IPS\cms\Databases\Controller
{
	...
	public function view()
	{
		...
		$table = new \IPS\Helpers\Table\Content( 'IPS\cms\Records' . \IPS\cms\Databases\Dispatcher::i()->databaseId, $category->url(), $where, $category, NULL, 'view', isset( \IPS\Request::i()->rss ) ? FALSE : TRUE );
		...
		$table->sortOptions = array_merge( $table->sortOptions, array_combine( array_keys( $sortBy ), array_keys( $sortBy ) ) );
			...
			if ( isset( \IPS\Request::i()->rss ) )
			{
				...
				foreach ( $table->getRows( array() ) as $record )
				{
					if ( ! $record->hidden() )
					{
						...
						$document->addItem( $record->_title, $record->url(), $content, \IPS\DateTime::ts( $record->record_last_comment ), $record->_id );
					}
				}
