...
class _updaterecords extends \IPS\Task
{
	...
	public function execute()
	{
		foreach( \IPS\cms\Databases::databases() as $database )
		{
			...
			{
				foreach( $recordClass::getItemsWithPermission( array( 'record_expiry_date > 0 and record_expiry_date <= ? and record_approved=1', time() ), 'primary_id_field ASC', 250, NULL ) as $record )
				{
					$record->hide( FALSE );
				}
			}
