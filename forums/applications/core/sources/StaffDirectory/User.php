...
class _User extends \IPS\Node\Model
{
	...
	public function formatFormValues( $values )
	{
		...
		if ( isset( $values['leader_use_custom_title'] ) )
		{
		...
		if( isset( $values['leader_custom_title'] ) AND isset( $values['leader_use_custom_title'] ) AND $values['leader_use_custom_title'] )
		{
	...
	protected function get__title()
	{
		...
		if ( $this->type === 'm' )
		{
			...
			{
				return \IPS\Member::loggedIn()->language()->addToStack( "core_staff_directory_name_{$this->id}" );
			}

			return \IPS\Member::load( $this->type_id )->name;
		}
		else
		{
	 ...
	 * @return	void
	 */
	public static function updateEmptySetting()
	{
		\IPS\Db::i()->update( 'core_sys_conf_settings', array( 'conf_value' => !( (bool) \IPS\Db::i()->select( 'COUNT(*)', 'core_leaders' )->first() ) ), array( 'conf_key=?', 'staff_directory_empty' ) );
