...
{	
	...
	public function _login( $data )
	{
		...
		{		
			...
			try
			{
				...
				switch ( $response->httpResponseCode )
				{
					case 200:
						...
						{
							\IPS\Log::i( LOG_ERR )->write( (string) $response );
			...
			{
				\IPS\Log::i( LOG_ERR )->write( get_class( $exception ) . "\n" . $exception->getCode() . ": " . $exception->getMessage() . "\n" . $exception->getTraceAsString() );
				$form->error = \IPS\Member::loggedIn()->language()->addToStack('download_upgrade_error');
			}
		}
	...
	public function _extractUpdate( $data )
	{
			...
			return (string) new \IPS\Helpers\MultipleRedirect( $url, function( $mrData ) use ( $data )
			{
				...
				elseif ( $mrData['status'] == 'download' )
				{
					...
					else
					{
						...
						else
						{
							...
							$response = \IPS\Http\Url::ips("download/{$data['key']}")->request( 30 )->setHeaders( array( 'Range' => "bytes={$startRange}-{$endRange}" ) )->get();
							
							try
							{
								\IPS\Log::i( LOG_ERR )->setIpAddress( $_SERVER['REMOTE_ADDR'] )->write( "Fetching download [range={$startRange}-{$endRange}] with a response code: " . $response->httpResponseCode, 'auto_upgrade' );
							}
							catch( \Exception $ex ) { }
							...
							if ( $response->httpResponseCode == 404 )
							{
								...
								try
								{
									\IPS\Log::i( LOG_ERR )->setIpAddress( $_SERVER['REMOTE_ADDR'] )->write( "Cannot fetch delta download: " . var_export( $response, TRUE ), 'auto_upgrade' );
								}
								catch( \Exception $ex ) { }
				...
				elseif ( $mrData['status'] == 'extract' )
				{
					try
					{
						...
						try
						{
							\IPS\Log::i( LOG_ERR )->setIpAddress( $_SERVER['REMOTE_ADDR'] )->write( "Attempting to extract", 'auto_upgrade' );
						}
						catch( \Exception $ex ) { }
						...
						else
						{
							...
							try
							{
								\IPS\Log::i( LOG_ERR )->setIpAddress( $_SERVER['REMOTE_ADDR'] )->write( "Extracting files {$mrData['extractNumber']} / {$numberOfFiles}", 'auto_upgrade' );
							}
							catch( \Exception $ex ) { }
					...
					catch ( \RuntimeException $e )
					{
						try
						{
							\IPS\Log::i( LOG_ERR )->setIpAddress( $_SERVER['REMOTE_ADDR'] )->write( "Extract Failed", 'auto_upgrade' );
							\IPS\Log::i( LOG_ERR )->setIpAddress( $_SERVER['REMOTE_ADDR'] )->write( $e, 'auto_upgrade' );
						}
						catch( \Exception $ex ) { }
		...
		try
		{
			...
			if ( count( $files ) )
			{
				...
				try
				{
					\IPS\Log::i( LOG_ERR )->setIpAddress( $_SERVER['REMOTE_ADDR'] )->write( "MD5 check of delta download failed with " . count( $files ) . " reported as modified", 'auto_upgrade' );
				}
				catch( \Exception $ex ) { }
