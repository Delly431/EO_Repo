...
class _members extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		$table->joins = array(
			...
			array(
				...
				'where' => array( 'g.row_id', \IPS\Db::i()->select( 'row_id', array( 'core_admin_permission_rows', 'sub' ), array( "((sub.row_id=core_members.member_group_id OR FIND_IN_SET( sub.row_id, core_members.mgroup_others ) ) AND sub.row_id_type='group') OR (sub.row_id=core_members.member_id AND sub.row_id_type='member')" ), NULL, array( 0, 1 ) ) ) ),
		);
		...
		$table->filters = array(
			...
			'members_filter_administrators'	=> 'g.row_id IS NOT NULL'
		);
			...
			array(
				...
				{										
					...
					$matrix->columns = array(
						...
						'import_as'	=> function( $key, $value, $data )
						{
							...
							foreach ( \IPS\core\ProfileFields\Field::fields( array(), \IPS\core\ProfileFields\STAFF ) as $groupId => $fields )
							{
				...
				'import_do_import'	=> function( $wizardData )
				{
					return (string) new \IPS\Helpers\MultipleRedirect( \IPS\Http\Url::internal('app=core&module=members&controller=members&do=import'),	function( $mrData ) use ( $wizardData )
					{
						...
						try
						{
							$password = mb_substr( md5( uniqid() ), 0, 8 );
							$member = new \IPS\Member;
	...
	public function export()
	{
		...
		$columns = array(
			...
			'name'					=> 'username',
			'email'					=> 'email',
			'password'				=> 'export_member_password',
			...
			array(
				...
				'export_choose_data'	=> function( $wizardData ) use ( $columns )
				{
					...
					if ( $values = $form->values() )
					{
						...
						foreach ( $wizardData['columns'] as $column )
						{
							if ( $column === 'password' )
							{
								$headers[] = 'password_hash';
								$headers[] = 'password_salt';
							}
							else
							{
								$headers[] = $column;
				...
				'export_build_list'		=> function( $wizardData ) use ( $columns )
				{
					...
					return (string) new \IPS\Helpers\MultipleRedirect(
						$baseUrl,
						function( $mrData ) use ( $wizardData )
						{
							$doPerLoop = 50;
							...
							/* Compile query */
							$query = \IPS\Db::i()->select( implode( ',', $select ), 'core_members', $where, 'core_members.member_id', array( $mrData, $doPerLoop ), NULL, NULL, \IPS\Db::SELECT_SQL_CALC_FOUND_ROWS );
							if ( count( $customFields ) )
							{
							...
							foreach ( $query as $member )
							{
								...
								foreach ( $wizardData['columns'] as $column )
								{
									...
									{
										case 'password':
											$dataToWrite[] = $member['members_pass_hash'];
											$dataToWrite[] = $member['members_pass_salt'];
											break;
										
										case 'primary_group_name':
											try
											{
												$dataToWrite[] = \IPS\Member::loggedIn()->language()->get( 'core_group_' . $member['member_group_id'] );
											}
											catch ( \UnderflowException $e )
											{
												$dataToWrite[] = '';
										...
										case 'secondary_group_names':
											...
											$dataToWrite[] = implode( ',', $secondaryGroupNames );
											break;
										
										case 'joined':
										case 'last_visit':
										case 'last_post':
											$dataToWrite[] = $member[ $column ] ? date( 'Y-m-d H:i', $member[ $column ] ) : '';
											break;
											
										case 'birthday':
											if ( $member['bday_day'] and $member['bday_month'] )
											{
												$dataToWrite[] = ( $member['bday_year'] ?: '????' ) . '-' . str_pad( $member['bday_month'], 2, '0', STR_PAD_LEFT ) . '-' . str_pad( $member['bday_day'], 2, '0', STR_PAD_LEFT );
											}
											else
											{
												$dataToWrite[] = '';
										...
										case 'skin_name':
											...
											try
											{
												$dataToWrite[] = \IPS\Member::loggedIn()->language()->get( 'core_theme_set_title_' . $themeId );
											}
											catch ( \UnderflowException $e )
											{
												$dataToWrite[] = '';
										...
										case 'language_name':
											...
											try
											{
												$dataToWrite[] = \IPS\Lang::load( $langId )->_title;
											}
											catch ( \OutOfRangeException $e )
											{
												$dataToWrite[] = '';
										...
										default:
											if ( mb_substr( $column, 0, 7 ) == 'pfield_' )
											{
												$dataToWrite[] = $member[ 'field_' . mb_substr( $column, 7 ) ];
											}
											else
											{
												$dataToWrite[] = $member[ $column ];
											}
											break;
									}
								}
							...
							fclose( $fh );
							$mrData += $doPerLoop;
							return array( $mrData, \IPS\Member::loggedIn()->language()->addToStack('export_members_processing'), 100 / $query->count( TRUE ) * $mrData );
						},
						function() use ( $baseUrl )
						{
							\IPS\Output::i()->redirect( $baseUrl->setQueryString( 'buildDone', 1 ) );
						}
					);
				},
				...
				'export_download_file'	=> function( $wizardData )
				{
					...
					return \IPS\Theme::i()->getTemplate( 'members' )->downloadMemberList();
				}
			),
