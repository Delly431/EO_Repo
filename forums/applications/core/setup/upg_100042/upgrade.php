...
class _Upgrade
{
	...
	public function step1()
	{
		...
		foreach( \IPS\Db::i()->query("SELECT *, COUNT( CONCAT_WS(';', css_set_id, css_app, css_location, css_path, css_name) ) as count FROM {$prefix}core_theme_css GROUP BY css_set_id, css_app, css_location, css_path, css_name ORDER BY count DESC" ) as $row )
		{
			if ( $row['count'] < 2 )
			{
				break;
	...
	public function step3()
	{
		...
		if( count( $queries ) )
		{
			...
			if ( count( $toRun ) )
			{
				$mr = \IPS\core\Setup\Upgrade::adjustMultipleRedirect( array( 1 => 'core', 'extra' => array( '_upgradeStep' => 4 ) ) );
