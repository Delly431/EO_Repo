	...
	public function step1()
	{
		...
		if ( count( $select ) )
		{
			...
			foreach ( $select as $row )
			{
					...
					if ( is_array( $membersCache ) and isset( $membersCache['notifications'] ) )
					{
						...
						if ( count( $notifications ) )
						{
							foreach( $notifications as $type => $selected )
							{
								try
								{
									/* @link http://community.invisionpower.com/resources/bugs.html/_/4-0-0/error-upgrade-data-truncated-r49262 */
				...
				/* http://community.invisionpower.com/4bugtrack/temp_ban-issues-while-3x-to-4-upgrade-r1404/ */
	...
	public function step7()
	{
		try
		{
			...
			foreach( $notifications as $key => $data )
			{
				...
				/* @link http://community.invisionpower.com/4bugtrack/beta-4a-upgrade-error-location-step-7-application-core-r937/ */
				$selected	= '';
...
DROP INDEX like_lookup_area"
		...
		switch( $step )
		{
			case 'polls':
				foreach( \IPS\Db::i()->select( '*', 'core_polls', NULL, 'pid ASC', array( $limit, $perCycle ) ) as $poll )
				{
					...
					/* @link http://community.invisionpower.com/4bugtrack/beta-8-json_encode-invalid-utf-8-sequence-in-argument-during-upgrade-from-347-r2636/
					...
					else if( $settings['idm_filestorage'] == 'db' )
					{
						...
						if( \IPS\Db::i()->checkForTable( 'downloads_files_records' ) )
						{
							\IPS\Db::i()->insert( 'core_files',
								\IPS\Db::i()->select( "NULL, downloads_files_records.record_realname, MD5( RAND() ), downloads_filestorage.storage_file, NULL", 'downloads_filestorage' )->join( 'downloads_files_records', "downloads_filestorage.storage_id=downloads_files_records.record_id" )
							);

							\IPS\Db::i()->insert( 'core_files',
								\IPS\Db::i()->select( "NULL, downloads_files_records.record_realname, MD5( RAND() ), downloads_filestorage.storage_ss, NULL", 'downloads_filestorage' )->join( 'downloads_files_records', "downloads_filestorage.storage_id=downloads_files_records.record_id" )
							);

							\IPS\Db::i()->insert( 'core_files',
								\IPS\Db::i()->select( "NULL, downloads_files_records.record_realname, MD5( RAND() ), downloads_filestorage.storage_thumb, NULL", 'downloads_filestorage' )->join( 'downloads_files_records', "downloads_filestorage.storage_id=downloads_files_records.record_id" )
							);
						}
				...
				foreach( $tables[ $table ]['columns'] as $column )
				{
					if( $table === 'core_attachments' )
					{
						if( mb_strpos( $row[ $column ], '.ipb' ) !== FALSE AND file_exists( $settings['upload_dir'] . '/' . $row[ $column ] ) )
						{
							...
							$newPathname = ltrim( implode( '/', $pathDetails ) . '/' . $row['attach_file'] . '.' . md5( uniqid( microtime() ) ), '/' );
	...
	public function step21()
	{
		...
		/* http://community.invisionpower.com/resources/bugs.html/_/ip-board/warn-logs-vs-members-warn-logs-r42251 */
