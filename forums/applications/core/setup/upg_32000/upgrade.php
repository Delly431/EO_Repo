...
class _Upgrade
{
	...
	'cookies', 'ipbreg', 'twitterconnect' )" );
		...
		$toRun = \IPS\core\Setup\Upgrade::runManualQueries( array( array(
			...
			'query' => "CREATE TABLE " . \IPS\Db::i()->prefix . "reputation_cache2 (SELECT * FROM " . \IPS\Db::i()->prefix . "reputation_cache WHERE id > 0 GROUP BY app, type, type_id);"
		),
		array(
			'table' => 'reputation_cache',
		...
		array(
			...
			'query' => "ALTER TABLE " . \IPS\Db::i()->prefix . "reputation_cache ADD PRIMARY KEY(id), 
				...
				ADD UNIQUE INDEX app (app, type, type_id),
		...
		\IPS\Db::i()->createTable( array(
			...
			'indexes'	=> array(
				...
				array(
					...
					'columns'	=> array( 'eas_member_id', 'eas_app', 'eas_section' )
				),
				array(
					'type'		=> 'key',
					'name'		=> 'eas_updated',
		...
		\IPS\Db::i()->createTable( array(
			...
			'indexes'	=> array(
				...
				array(
					'type'		=> 'key',
					'name'		=> 'tag_lookup',
					'columns'	=> array( 'tag_perm_text', 'tag_perm_visible' ),
				),
			)
		)	);
	...
	'happy.gif', 'huh.gif', 'laugh.gif', 'mellow.gif', 'ohmy.gif', 'ph34r.gif', 'sad.gif', 'sleep.gif', 'smile.gif', 'tongue.gif', 'unsure.gif', 'wacko.gif', 'wink.gif', 'wub.gif' );" );
		...
		$toRun = \IPS\core\Setup\Upgrade::runManualQueries( array( array(
			...
			'query' => "ALTER TABLE " . \IPS\Db::i()->prefix . "core_like ADD COLUMN like_lookup_area CHAR(32) NOT NULL DEFAULT '', 
				...
				ADD INDEX notification_task ( like_notify_do, like_app(50), like_area(50), like_visible, like_notify_sent, like_notify_freq );"
		),
		array(
			'table' => 'core_like',
			'query' => "UPDATE " . \IPS\Db::i()->prefix . "core_like SET like_lookup_area = MD5( CONCAT( like_app, ';', like_area, ';', like_member_id ) );"
		),
		array(
		...
		if ( count( $toRun ) )
		{
			$mr = \IPS\core\Setup\Upgrade::adjustMultipleRedirect( array( 1 => 'core', 'extra' => array( '_upgradeStep' => 3 ) ) );
	...
	SELECT comment_for_member_id, comment_by_member_id, comment_date, comment_content, 0, '', 0, 0, MD5(comment_content), 0, '', comment_ip_address, comment_approved FROM " . \IPS\Db::i()->prefix . "profile_comments" );
		...
		if ( count( $toRun ) )
		{
			$mr = \IPS\core\Setup\Upgrade::adjustMultipleRedirect( array( 1 => 'core', 'extra' => array( '_upgradeStep' => 5 ) ) );
	...
	public function step6()
	{
		...
		if ( count( $toRun ) )
		{
			$mr = \IPS\core\Setup\Upgrade::adjustMultipleRedirect( array( 1 => 'core', 'extra' => array( '_upgradeStep' => 7 ) ) );
	...
	ADD INDEX queued (queued,pdelete_time);";
		...
		/* If we have a lot of posts, prompt to run manually */
		if( \IPS\Db::i()->select( 'count(*)', 'posts' )->first() > 100000 )
		{
			$mr = \IPS\core\Setup\Upgrade::adjustMultipleRedirect( array( 1 => 'core', 'extra' => array( '_upgradeStep' => 8 ) ) );

			/* Queries to run manually */
			return array( 'html' => \IPS\Theme::i()->getTemplate( 'forms' )->queries( array( $query ), \IPS\Http\Url::internal( 'controller=upgrade' )->setQueryString( array( 'key' => $_SESSION['uniqueKey'], 'mr_continue' => 1, 'mr' => $mr ) ) ) );
		}

		/* @note Running as a direct query() in order to combine all of the changes into one query for resource reasons */
